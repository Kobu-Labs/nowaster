...
def get_track_entries_by_category(
    db: Session, category: str
) -> List[models.TrackEntry]:
    return (
        db.query(models.TrackEntry).filter(models.TrackEntry.category == category).all()
    )
...
def create_track_entry(
    db: Session, track_entry_create: schemas.TrackEntryCreate
) -> models.TrackEntry:
    start_date = datetime.now()
    end_date = datetime.now() + timedelta(hours=track_entry_create.duration)
    track_entry = models.TrackEntry(
